...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN"):
        ...
        self.emb_layer = nn.Embedding(output_size,emb_size)
        if(model_type=="RNN"):
            self.model_layer = nn.RNN(emb_size, decoder_hidden_size, 1, dropout = dropout, batch_first=True)
        else:
            self.model_layer = nn.LSTM(emb_size, decoder_hidden_size, 1, dropout = dropout, batch_first=True)
    ...
    def forward(self, input, hidden):
        ...
        #############################################################################
        embeds = self.dropout(self.emb_layer(input))
        if(self.model_type == "RNN"):
            output, hidden = self.model_layer(embeds, hidden)
        if(self.model_type == "LSTM"):
            output, (hidden, cell_state) = self.model_layer(embeds, hidden)
